...
\begin{example}
	Using the general state-space equaions and the definition of $h(n)$, derive the convolutional equation.
\end{example}

\begin{solution}
	For any input $x(n)$, out state space equations look like:
...
\begin{itemize}
	\item The \emph{effect} of a system is defined as
	\item The convolutional equation tells us how the effect of a system evolved over time.
\end{itemize}
...
\begin{solution}
	From a previous question, we determined the inpulse response of this sytem to be \\ $h(n) = \begin{cases} \frac{1}{2}, \; \text{if} \; n = 0, 1 \\ 0, \; \text{otherwise} \end{cases}$. Thus, using the convolutional equation, we can see that:
...
\begin{itemize}
	\item The identity system is defined as a system where the impulse response, $h(n)$, is the Kroncker-Delta function, $\delta(n)$.
\end{itemize}

\begin{example}
	Find the output of the system $y(n) = x(n) + \alpha{y(n - 1)}$ for the input $x(n) = 1$ using its impilse response.
\end{example}
